...
elseif GetLocale() == "deDE" then
	...
	ERROR_INDEX = "Der zur Verfugung gestellte Index muß eine Zahl sein."
	...
	ADDON_CALL_PROTECTED = "[%s] AddOn '%s' hat versucht die geschutzte Funktion '%s' aufzurufen."
	ADDON_CALL_PROTECTED_MATCH = "^%[(.*)%] (AddOn '.*' hat versucht die geschutzte Funktion '.*' aufzurufen.)$"
	ADDON_DISABLED = "|cffffff7fBugGrabber|r und |cffffff7f%s|r konnen nicht zusammen laufen, |cffffff7f%s|r wurde deshalb deaktiviert. Du kannst jetzt WoW schließen, |cffffff7fBugGrabber|r deaktivieren und |cffffff7f%s|r erneut aktivieren."
...
elseif GetLocale() == "zhTW" then
	...
	ERROR_INDEX = "提供的索引?必須是數字。"
	ERROR_UNKNOWN_INDEX = "提供的索引?「%d」不是正確的。"
	STARTUP_ERRORS = "在?動時發生%d個錯誤："
	STARTUP_ERRORS_MANY = "在?動時發生%d個錯誤，請使用 /buggrabber <索引?> 來列出。"
	UNIQUE_CAPTURE = "捕捉到新的錯誤：\n%s\n---"
	ADDON_CALL_PROTECTED = "[%s] ?件 '%s' 嘗試調用保護功能 '%s'。"
	ADDON_CALL_PROTECTED_MATCH = "^%[(.*)%] (?件 '.*' 嘗試調用保護功能 '.*'.)$"
	ADDON_DISABLED = "|cffffff7fBugGrabber|r 和 |cffffff7f%s|r 不能共存。|cffffff7f%s|r 已停用。可在?件介面停用 |cffffff7fBugGrabber|r，再用 |cffffff7f%s|r。"
	BUGGRABBER_STOPPED = "|cffffff7fBugGrabber|r 現正暫停，因?每秒捕捉到超過%d個錯誤。?會在%d秒後重新開始。"
	BUGGRABBER_RESUMING = "|cffffff7fBugGrabber|r 已重新開始。"
elseif GetLocale() == "zhCN" then
	CMD_CREATED = "??一???，用 /buggrabber 列出????。"
	USAGE = "用法：/buggrabber <1-%d>。"
	ERROR_INDEX = "提供的索引?必?是?字。"
	ERROR_UNKNOWN_INDEX = "提供的索引?「%d」不是正?的。"
	STARTUP_ERRORS = "在????生%d???："
	STARTUP_ERRORS_MANY = "在????生%d???，?使用 /buggrabber <索引?>?列出。"
	UNIQUE_CAPTURE = "BugGrabber 捕捉到新的??：\n%s\n---"
	ADDON_CALL_PROTECTED = "[%s] ?件 '%s' ???用保?功能 '%s'。"
	ADDON_CALL_PROTECTED_MATCH = "^%[(.*)%] (?件 '.*' ???用保?功能 '.*'.)$"
	ADDON_DISABLED = "|cffffff7fBugGrabber|r 和 |cffffff7f%s|r 不能共存。|cffffff7f%s|r 已停用。可在?件界面停用 |cffffff7fBugGrabber|r 再用 |cffffff7f%s|r。"
	BUGGRABBER_STOPPED = "|cffffff7fBugGrabber|r ?正?停，因?每秒捕捉到超?%d???。??在%d秒后重新?始。"
	BUGGRABBER_RESUMING = "|cffffff7fBugGrabber|r 已重新?始。"
...
elseif GetLocale() == "frFR" then
	CMD_CREATED = "Une erreur a ete detectee, tapez /buggrabber pour l'afficher."
	USAGE = "Utilisation: /buggrabber <1-%d>."
	ERROR_INDEX = "L'element donne doit etre un nombre."
	ERROR_UNKNOWN_INDEX = "L'element %d n'existe pas dans la table d'erreurs chargee."
	STARTUP_ERRORS = "Il y a eu %d erreurs au demarrage:"
	STARTUP_ERRORS_MANY = "Il y a eu %d erreurs au demarrage, merci d'utiliser /buggrabber <nombre> pour les afficher."
	--UNIQUE_CAPTURE = "BugGrabber a capture une seule erreur:\n%s\n---"
	ADDON_CALL_PROTECTED = "[%s] L'AddOn '%s' a tente d'appeler la fonction protegee '%s'."
	ADDON_CALL_PROTECTED_MATCH = "^%[(.*)%] (L'AddOn '.*' a tente d'appeler la fonction protegee '.*'.)$"
	ADDON_DISABLED = "|cffffff7fBugGrabber|r et |cffffff7f%s|r ne peuvent pas etre lances en meme temps. |cffffff7f%s|r a ete desactive. Si vous le souhaitez, vous pouvez vous deconnecter, desactiver |cffffff7fBugGrabber|r et reactiver |cffffff7f%s|r."
	BUGGRABBER_STOPPED = "|cffffff7fBugGrabber|r a cesse de capturer des erreurs, car plus de %d erreurs ont ete capturees par seconde. La capture sera reprise dans %d secondes."
	BUGGRABBER_RESUMING = "|cffffff7fBugGrabber|r capture les erreurs a nouveau."
end
